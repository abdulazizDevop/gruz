  ...
  return (
    <div className="min-h-screen bg-[#09090b] text-white flex">
      ...
      <motion.aside
        ...
        className="fixed top-0 left-0 h-screen bg-[#0f0f12] border-r border-white/[0.06] flex flex-col z-50"
      >
        {/* Logo */}
        <div className={`h-16 flex items-center border-b border-white/[0.06] ${isSidebarOpen ? 'px-5 gap-3' : 'justify-center'}`}>
          <img src="/doorman-logo.png?v=3" alt="D" className="w-8 h-8 object-contain" />
          {isSidebarOpen && (
            <span
          ...
          <button
            ...
            className={`p-1.5 hover:bg-white/5 rounded-lg text-gray-500 hover:text-gray-300 transition-colors ${isSidebarOpen ? 'ml-auto' : 'hidden'}`}
          >
            <ChevronLeft size={16} />
          </button>
        </div>
        ...
        {!isSidebarOpen && (
          <button
            ...
            className="flex items-center justify-center py-3 text-gray-500 hover:text-gray-300"
          >
            <Menu size={18} />
          </button>
        )}
        ...
        {/* Nav */}
        <nav className={`flex-1 ${isSidebarOpen ? 'px-3' : 'px-2'} py-4 space-y-1 overflow-y-auto`}>
          {isSidebarOpen && <p className="text-[10px] font-semibold text-gray-600 uppercase tracking-widest px-4 mb-2">Главное</p>}
          <SidebarItem to="/" icon={LayoutDashboard} label="Обзор" active={location.pathname === '/'} collapsed={!isSidebarOpen} />

          {(isAdmin || isSuperAdmin || isAssembler) && (
            <>
              {isSidebarOpen && <p className="text-[10px] font-semibold text-gray-600 uppercase tracking-widest px-4 mt-5 mb-2">Торговля</p>}
              <SidebarItem to="/orders" icon={ShoppingCart} label="Заказы" active={location.pathname === '/orders'} collapsed={!isSidebarOpen} />
              {(isAdmin || isSuperAdmin) && (
                <SidebarItem to="/wholesalers" icon={Users} label="Оптовики" active={location.pathname === '/wholesalers'} collapsed={!isSidebarOpen} />
              )}
            </>
          )}
          ...
          {(isWarehouse || isSuperAdmin) && (
            <>
              {isSidebarOpen && <p className="text-[10px] font-semibold text-gray-600 uppercase tracking-widest px-4 mt-5 mb-2">Склад</p>}
              <SidebarItem to="/warehouse" icon={Warehouse} label="Склад" active={location.pathname === '/warehouse'} collapsed={!isSidebarOpen} />
            </>
          )}

          <SidebarItem to="/reserved" icon={PackageCheck} label="Заказной склад" active={location.pathname === '/reserved'} collapsed={!isSidebarOpen} />

          {isSuperAdmin && (
            <>
              {isSidebarOpen && <p className="text-[10px] font-semibold text-gray-600 uppercase tracking-widest px-4 mt-5 mb-2">Система</p>}
              <SidebarItem to="/admins" icon={UserCheck} label="Управление" active={location.pathname === '/admins'} collapsed={!isSidebarOpen} />
            </>
          )}
        </nav>
        ...
        <div className={`border-t border-white/[0.06] p-3`}>
          <div className={`flex items-center gap-3 p-2 rounded-xl ${isSidebarOpen ? '' : 'justify-center'}`}>
          ...
          <button
            onClick={logout}
            className={`w-full mt-1 flex items-center gap-2 px-3 py-2.5 text-gray-500 hover:text-red-400 hover:bg-red-500/5 rounded-xl transition-colors text-sm ${!isSidebarOpen ? 'justify-center' : ''}`}
          >
            <LogOut size={18} />
      ...
      >
        {/* Top bar */}
        <header className="h-16 border-b border-white/[0.06] flex items-center justify-between px-6 bg-[#09090b]/90 backdrop-blur-xl sticky top-0 z-40">
          ...
          <div className="flex items-center gap-3">
            {readyCount > 0 && (
              <Link to="/reserved" className="flex items-center gap-2 px-3 py-1.5 bg-amber-500/10 border border-amber-500/20 rounded-xl hover:bg-amber-500/15 transition-colors">
                <AlertCircle size={14} className="text-amber-500" />
                <span className="text-xs font-semibold text-amber-500">{readyCount} к отгрузке</span>
              </Link>
            )}
